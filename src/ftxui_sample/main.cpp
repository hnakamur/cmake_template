...
template <std::size_t Width, std::size_t Height> struct GameBoard {
  ...
  std::string &
  get_string(std::size_t cur_x, std::size_t cur_y)
  {
    return strings.at(cur_x).at(cur_y);
  }

  void
  set(std::size_t cur_x, std::size_t cur_y, bool new_value)
  {
  ...
  [[nodiscard]] bool
  get(std::size_t cur_x, std::size_t cur_y) const
  {
    return values.at(cur_x).at(cur_y);
  }

  [[nodiscard]] bool &
  get(std::size_t cur_x, std::size_t cur_y)
  {
    return values.at(cur_x).at(cur_y);
  }
  ...
  void
  toggle(std::size_t cur_x, std::size_t cur_y)
  {
    set(cur_x, cur_y, !get(cur_x, cur_y));
  }

  void
  press(std::size_t cur_x, std::size_t cur_y)
  {
    ++move_count;
    toggle(cur_x, cur_y);
    if (cur_x > 0) {
...
struct Bitmap : ftxui::Node {
  ...
  Color &
  at(std::size_t cur_x, std::size_t cur_y)
  {
    return pixels.at((width_ * cur_y) + cur_x);
  }

  void
  ComputeRequirement() override
  {
  ...
  void
  Render(ftxui::Screen &screen) override
  {
...
game_iteration_canvas()
{
  ...
  auto make_layout = [&] {
    ...
    return ftxui::hbox({bm | ftxui::border, ftxui::vbox({ftxui::text("Frame: " + std::to_string(counter)),
                                                         ftxui::text("FPS: " + std::to_string(fps)), small_bm | ftxui::border})});
  };
